...
var USER_INDEX = (function () {
    ...
    function loadClients(operatorId, date) {
        $.ajax({
            ...
            "crossOrigin": true
            ,
            // url: "http://" + serverLoc + ":8080/web-service-demo-1.0/clients?status=3" старе
            url: "http://" + serverLoc + ":8080/web-service-demo-1.0/pool?status=3&date=" + moment().format("YYYY-MM-DD")
            ,
            headers: {
    ...
    function visitsTable(data, element, radioName, clientCard) {
        ...
        if (radioName != null) {
            getVisit(clientCard.id, function (visitData) {
    ...
    function statusModifications(clientCard, value) {
        ...
        if (value == 7) {
            // $('#btnBlockList').hide();
            $(".status-row td:nth-child(2)").removeClass().addClass("primary");
        }
    ...
    function reserve(clientCard) {
        ...
        $('#reserveVisitsToday').unbind('click').click(function () {
            ...
            if (value != null) {
                if (moment(visitDateTime).isAfter(moment().format("YYYY-MM-DDTHH:mm:ssZ"))) {
    ...
    function loadClientCard(clientCard) {
        ...
        $('#submitConfirmClient').unbind('click').click(function () {
            $("#status").editable('setValue', 6);
        });

        $('#submitStarClient').unbind('click').click(function () {
            $("#status").editable('setValue', 7);
        });
        ...
        $('#submitModal').unbind("click").click(function () {
            ...
            // var num2 = $("#number2").editable('getValue').number2;
            // if(num2 == ""){
            //     clientCard.client.number2 = null;
            // }else {
            //     clientCard.client.number2 = num2;
            // }
            //
            // var num3 = $("#number3").editable('getValue').number3;
            // if(num3 == ""){
            //     clientCard.client.number3 = null;
            // }else {
            //     clientCard.client.number3 = num3;
            // }
            ...
            //clientCard.statusDate = $("#statusDate").editable('getValue').statusDate;
            ...
            var infoData = $("#info").editable('getValue').info;
            if(infoData == ""){
                clientCard.client.info = null;
            }else {
                clientCard.client.info = infoData;
            }
